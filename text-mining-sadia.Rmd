...
#Sentiments score per word of the books
```{r}
...
````
#Total sentiment score of each book
```{r}
Bible_total<-Bible1 %>% inner_join(get_sentiments("afinn")) %>% summarise(total= sum(score))
Picture_total<-picture.clean %>% inner_join(get_sentiments("afinn")) %>% summarise(total= sum(score))
...
total<-rbind(Bible_total,Buddhism_total, Koran_total, Picture_total, Alice_total) %>% mutate(book=c("Bible","The Essence of Buddhism","Koran", "Picture of Dorian Gray", "Alice's Adventures in Wonderland")) 

#plot
ggplot(total,aes(x=reorder(book, total),y=total,fill=book)) + 
  geom_bar(stat = "identity", show.legend = FALSE) + 
  xlab("Book") + 
  ylab("Total score") + 
...
Picture<-picture.clean %>% inner_join(get_sentiments("bing")) %>% mutate(book="picture")
Buddhism<-Buddhism1 %>% mutate(linenumber = row_number()) %>% inner_join(get_sentiments("bing")) %>% mutate(book="buddhism")
Koran<-Koran1 %>% mutate(linenumber = row_number()) %>% inner_join(get_sentiments("bing")) %>% mutate(book="koran")
Alice<-alice_tidy %>% inner_join(get_sentiments("bing")) %>% mutate(book="alice")

books<-bind_rows(Bible,Buddhism, Koran, Picture, Alice) 
...
books %>%
  ...
  labs(x="Sections of the book", y="Overall sentiments", title="Change of sentiments throughout the books", fill="overall sentiment =\npositive-negative") +
  theme(plot.title = element_text(hjust = 0.5))
