...
We begin by downloading the books with the "gutenberg_download" function of gutenbergr package. "gutenbergr" package has other functions like:
...
- The 5 downloaded books are saved in a list `booklist.rda` in the `data_SS` folder.
...
We can take the `.rda` file and look at the structure of the list:
...
Our next goal is to clean the books (removing spaces, punctuations, arranging by chapter) and to create a data frame of all books `tidy_books.rda`:
